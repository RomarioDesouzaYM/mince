"""
seed.py — MINCE demo data (REFERENCE DRAFT for checking the agent's output)

Seeds:
  - districts across Jayawijaya, Yalimo, Mamberamo Tengah, with MANUAL jarak / waktu
    tempuh / jenis_akses (these are the human-maintained fields, not Haversine)
  - ~25 demo reports spread by kabupaten, kategori (incl. Jaringan + Listrik), urgensi,
    status, so the map / risiko / ringkasan pages have something to show

IMPORTANT: coordinates and jarak/waktu values below are APPROXIMATE seed placeholders.
Because jarak & waktu tempuh are manual by design, replace them with real field values
before the demo. They are marked "verify".

Run:  python -m app.seed      (or however your project invokes it)
"""
    ...
    try:
        ...
        for i in range(total):
            ...
            report = Report(
                ...
                # jitter marker position slightly around the district center
